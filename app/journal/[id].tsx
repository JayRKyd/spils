...
export default function JournalDetail() {
  ...
  // Share a downloadable PDF summary sheet of the entry (via the OS share sheet)
  const handleShare = async () => {
    if (!entry) return;
    try {
      const esc = (v: unknown) => String(v ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
      const row = (label: string, value?: string | null) =>
        value && value.trim() ? `<tr><td class="l">${label}</td><td class="v">${esc(value)}</td></tr>` : "";
      const listVal = (arr?: string[] | null) => (arr?.length ? arr.join(", ") : "");
      const dd = new Date(entry.entry_date);
      const dateStr = `${String(dd.getMonth() + 1).padStart(2, "0")}.${String(dd.getDate()).padStart(2, "0")}.${String(dd.getFullYear()).slice(-2)}`;
      const img = entry.image_url && entry.image_url.startsWith("http")
        ? `<img class="bottle" src="${esc(entry.image_url)}" />` : "";
      const html = `
<html><head><meta charset="utf-8"><style>
  body { font-family: -apple-system, Helvetica, Arial, sans-serif; margin: 42px; color: #13131a; }
  .top { display: flex; justify-content: space-between; align-items: baseline; border-bottom: 2px solid #13131a; padding-bottom: 10px; }
  .logo { font-size: 20px; font-weight: 800; letter-spacing: 1.5px; }
  .sheetType { font-size: 10px; letter-spacing: 2px; color: #888; }
  .head { display: flex; justify-content: space-between; margin-top: 26px; }
  h1 { font-size: 26px; margin: 0 0 2px; }
  .brand { font-size: 14px; color: #666; margin: 0; }
  .date { font-size: 11px; color: #999; margin-top: 6px; }
  .bottle { width: 96px; height: 96px; object-fit: cover; border-radius: 10px; }
  .section { font-size: 10px; font-weight: 700; letter-spacing: 2px; color: #999; margin: 26px 0 6px; }
  table { width: 100%; border-collapse: collapse; }
  td { padding: 6px 0; border-bottom: 1px solid #eee; font-size: 12px; vertical-align: top; }
  .l { color: #888; width: 130px; text-transform: uppercase; font-size: 10px; letter-spacing: 1px; padding-top: 8px; }
  .v { text-align: right; font-weight: 600; }
  .body { font-size: 12px; line-height: 1.55; margin-top: 4px; white-space: pre-wrap; }
  .foot { margin-top: 34px; font-size: 9px; color: #bbb; letter-spacing: 1px; text-align: center; }
</style></head><body>
  <div class="top"><div class="logo">SP/LS.</div><div class="sheetType">SCENT JOURNAL SUMMARY</div></div>
  <div class="head">
    <div>
      <h1>${esc(entry.title || "Journal Entry")}</h1>
      <p class="brand">${esc(entry.brand ?? "")}</p>
  ...
  <table>
    ...
    ${row("Season(s)", listVal(entry.seasons))}
    ${row("Reminds me of", entry.reminds_me_of)}
  </table>
  ${entry.accords?.length ? `<div class="section">OLFACTIVE PROFILE</div><div class="body">${esc(listVal(entry.accords))}</div>` : ""}
  ${(entry.notes_top?.length || entry.notes_heart?.length || entry.notes_base?.length) ? `
  <div class="section">NOTES PYRAMID</div>
  <table>
    ${row("Top Notes", listVal(entry.notes_top))}
    ${row("Middle Notes", listVal(entry.notes_heart))}
    ${row("Base Notes", listVal(entry.notes_base))}
  </table>` : ""}
  <div class="section">PERFORMANCE</div>
  <table>
    ${row("Projection", entry.projection ? `${entry.projection} / 10` : null)}
    ${row("Sillage", entry.sillage ? `${entry.sillage} / 10` : null)}
    ${row("Longevity", entry.longevity ? `${entry.longevity} / 10` : null)}
  </table>
  ${entry.dry_down ? `<div class="section">DRYDOWN</div><div class="body">${esc(entry.dry_down)}</div>` : ""}
  ${entry.description ? `<div class="section">NOTES</div><div class="body">${esc(entry.description)}</div>` : ""}
  <div class="foot">GENERATED BY SPILS</div>
</body></html>`;
    ...
    <Wrapper>
      ...
      {entry && (
        <EditModal
          ...
          onSaved={() => { setEditVisible(false); setTimeout(() => router.back(), 400); }}
        />
      )}
      {/* More Sheet */}
      <Modal visible={moreVisible} transparent animationType="slide" onRequestClose={() => setMoreVisible(false)}>
        <View style={ms.backdrop}>
          ...
          <View style={ms.sheet}>
            ...
            <TouchableOpacity style={ms.btn} onPress={() => { setMoreVisible(false); setTimeout(handleAddToCollection, 400); }}>
