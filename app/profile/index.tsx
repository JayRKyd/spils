...
import {
  ...
  TextInput, ActivityIndicator, Modal, Alert, StyleSheet,
} from "react-native";
...
function EditProfileModal({ visible, profile, onClose, onSaved }: {
  ...
  useEffect(() => {
    if (visible) { setUsername(profile.username ?? ""); setBio(profile.bio ?? ""); }
  }, [visible, profile]);

  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    await (supabase as any).from("profiles").upsert({ id: user.id, username: username.trim() || null, bio: bio.trim() || null }, { onConflict: "id" });
    setSaving(false);
    onSaved();
  };
...
export default function ProfileScreen() {
  ...
  const [stats, setStats] = useState<Stats>({ listings: 0, messages: 0, watchlist: 0, perfumes: 0, formulas: 0, journal: 0 });
...
const s = StyleSheet.create({
  ...
  avatar: { width: 64, height: 64, borderRadius: 32, backgroundColor: "rgba(167,139,250,0.25)", borderWidth: 2, borderColor: "rgba(167,139,250,0.5)", alignItems: "center", justifyContent: "center" },
